        ...
        <div class="contentwrapper">
            <div class="details">
                <div class="search_input">
                    <p>
                        名称:<input type="text" name="role_name" id="role_name"/>
                    </p>
                    <p>
                        ...
                        <c:if test="${retInfo.obj.province_id == -1 || retInfo.obj.city_id == -1 || retInfo.obj.region_id == -1}">
                            ...
                            <c:forEach items="${retInfo.obj.dicts}" var="bean" >
                                    <option value="${bean.dict_id}">${bean.code_name}</option>
                            </c:forEach>
                            </select>
                        </c:if>
                        <c:if test="${retInfo.obj.divide_id == -1}">
                            <select id="divides">
                                <c:forEach items="${retInfo.obj.divides}" var="bean" >
                                    <option value="${bean.divide_id}">${bean.divide_name}</option>
                                </c:forEach>
                            </select>
                        </c:if>
                    ...
                    <div class="box">
                        <ul id="tree" class="filetree">
                            <c:forEach items="${functions.obj}" var="bean">
                                <li class="menu"><input type="checkbox" name="" value="${bean.func_id}">${bean.func_name}
                                    <c:if test="${bean.is_father == 0}">
                                        <ul>
                                            <c:forEach items="${bean.tdHtFuncionDtos}" var="bean1">
                                                <li class="menu"><input type="checkbox" name="" value="${bean1.func_id}">${bean1.func_name}
                                                    <c:if test="${bean1.is_father == 0}">
                                                        <ul>
                                                            <c:forEach items="${bean1.tdHtFuncionDtos}" var="bean2">
                                                                <li class="menu"><input type="checkbox" name="" value="${bean2.func_id}">${bean2.func_name}
                                                                    <c:if test="${bean2.is_father == 0}">
                                                                         <ul>
                                                                             <c:forEach items="${bean2.tdHtFuncionDtos}" var="bean3">
                                                                                 <li class="menu"><input type="checkbox" name="" value="${bean3.func_id}">${bean3.func_name}
                                                                                     <c:if test="${bean3.is_father == 0}">
                                                                                         <ul>
                                                                                             <c:forEach items="${bean3.tdHtFuncionDtos}" var="bean4">
                                                                                                 <li class="menu"><input type="checkbox" name="" value="${bean4.func_id}">${bean4.func_name}
                                                                                                     <c:if test="${bean4.is_father == 0}">
                                                                                                         <ul>
                                                                                                             <c:forEach items="${bean4.tdHtFuncionDtos}" var="bean5">
                                                                                                                 <li class="menu"><input type="checkbox" name="" value="${bean5.func_id}">${bean5.func_name}</li>
                                                                                                             </c:forEach>
                                                                                                         </ul>
                                                                                                     </c:if>
                                                                                                 </li>
