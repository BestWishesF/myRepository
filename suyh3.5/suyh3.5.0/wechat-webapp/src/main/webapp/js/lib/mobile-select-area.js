(function(root, factory) {
	...
	MobileSelectArea.prototype = {
		...
		bindEvent: function() {
			...
			this.trigger[_this.settings.eventName](function(e) {
				...
				_this.getData().done(function() {
					_this.format(e.target.dataset.province, e.target.dataset.city, e.target.dataset.region);//点击
				});
				var start = 0,
					end = 0
		...
		format: function(province, city, region) {
			var _this = this;
			var child = _this.scroller.children();
			this.f(this.data, province, city, region, 1);
		},
		f: function(data, province, city, region, index) {
			var _this = this;
			var item = data;
			if (!item) {
				item = [];
			...
			if (_this.index !== 0 && _this.value[_this.index - 1] == "0" && this.default == 0) {
				...
				for (var j = _this.default, len = item.length; j < len; j++) {
					...
					if(id == province && index == 1){//判断省id
                        cls = "focus";
                        focus = id;
                        childData = item[j].child;
                        top = _this.mtop * (-(j - _this.default));
					}
                    if(id == city && index == 2){//判断市id
                        cls = "focus";
                        focus = id;
                        childData = item[j].child;
                        top = _this.mtop * (-(j - _this.default));
                    }
                    if(id == region && index == 3){//判断区id
                        cls = "focus";
                        focus = id;
                        childData = item[j].child;
                        top = _this.mtop * (-(j - _this.default));
            ...
            index = index + 1;
			_this.f(childData, province, city, region, index);
		},
		submit: function() {
			this.oldvalue = this.value.concat([]);
			this.oldtext = this.text.concat([]);
