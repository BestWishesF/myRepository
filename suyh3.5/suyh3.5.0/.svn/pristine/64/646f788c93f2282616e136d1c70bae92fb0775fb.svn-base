            ...
            <div class="contentwrapper">
                <div class="details">
                    <div class="search_input">
                        <p>
                            快递网点名称:<input type="text" name="dot_name" id="dot_name" value="${retInfo.obj.dot_name}" />
                        </p>
                        <p>
                            快递网点编号:<input type="text" name="dot_number" id="dot_number" value="${retInfo.obj.dot_number}" />
                        </p>
                        <p>
                            分配客户名称:<input type="text" name="customer_name" id="customer_name" value="${retInfo.obj.customer_name}" />
                        </p>
                        <p>
                            分配客户编号:<input type="text" name="customer_number" id="customer_number" value="${retInfo.obj.customer_number}" />
                        </p>
                        <p>
                            分配客户密码:<input type="text" name="customer_password" id="customer_password" value="${retInfo.obj.customer_password}" />
                        </p>
                        <p>
                            快递接口类型:
                            <select id="sdkj_itf_type">
                                <option value="1" <c:if test="${retInfo.obj.sdkj_itf_type == 1}">selected</c:if>>提取单号</option>
                                <option value="2" <c:if test="${retInfo.obj.sdkj_itf_type == 2}">selected</c:if>>筛单</option>
                                <option value="3" <c:if test="${retInfo.obj.sdkj_itf_type == 3}">selected</c:if>>获取打印大头笔</option>
                                <option value="4" <c:if test="${retInfo.obj.sdkj_itf_type == 4}">selected</c:if>>提交订单</option>
                                <option value="5" <c:if test="${retInfo.obj.sdkj_itf_type == 5}">selected</c:if>>查询物流</option>
                                <option value="6" <c:if test="${retInfo.obj.sdkj_itf_type == 6}">selected</c:if>>获取可用单号数量</option>
                                <option value="7" <c:if test="${retInfo.obj.sdkj_itf_type == 7}">selected</c:if>>获取可用单号数量</option>
                            </select>
                        </p>
                        <p>
                            快递接口地址:<input type="text" name="sdk_itf_url" id="sdk_itf_url" value="${retInfo.obj.sdk_itf_url}" />
                        </p>
                        <p>
                            快递接口密钥:<input type="text" name="app_key" id="app_key" value="${retInfo.obj.app_key}" />
                        </p>
                        <p>
                            快递接口密文:<input type="text" name="app_secret" id="app_secret" value="${retInfo.obj.app_secret}" />
                        </p>
                        <p>
                            接口调用凭证:<input type="text" name="access_token" id="access_token" value="${retInfo.obj.access_token}" />
                        </p>
                        <p>
                            接口方法标识:<input type="text" name="sdk_code" id="sdk_code" value="${retInfo.obj.sdk_code}" />
                        </p>
                        <p>
...
<script type="text/javascript">
    ...
    $("#submit_bt").click(function () {
        var data = "dot_name=" + $("#dot_name").val() + "&dot_number=" + $("#dot_number").val() + "&customer_name=" +
            ...
            "&access_token=" + $("#access_token").val() + "&sdk_code=" + $("#sdk_code").val() + "&exp_sdk_id=${retInfo.obj.exp_sdk_id}";

        $.ajax({
            type:'post',
            url:'${ctx}/admin/express/sdk/update',
            dataType : 'json',
            data: data,
            contentType: "application/x-www-form-urlencoded; charset=utf-8",
            success:function(data){
                if(data.mark=="0"){
                    window.location.href="${ctx}/admin/express/sdk/list?currentPage=1&eoa_id=${retInfo.obj.eoa_id}";
                }else{
                    alert(data.tip);
                }
            }
