    ...
    <div class="centercontent tables">
        <div id="contentwrapper" class="contentwrapper">
                    ...
                    <c:when test="${funcion.func_name == '添加'}">
                        <div class="details">
                            <div class="search_input">
                                <p><button class="submit radius2" onclick="main('${ctx}${funcion.link_url}?code_type=${retInfo.obj.code_type}&parent_id=0&father_id=${funcion.father_id}')">添加</button></p>
                            </div>
                        </div><!--details-->
                    </c:when>
                </c:choose>
                    ...
                    <td>
                        <c:forEach items="${funcions.obj.sonFuncion}" var="funcion">
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <a  href="#"  onclick="main('${ctx}${funcion.link_url}?dict_id=${bean.dict_id}&currentPage=1&code_type=city&parent_id=${bean.dict_id}&express_id=${bean.dict_id}&region_id=${retInfo.obj.city}&exp_open_state=0&father_id=${funcion.func_id}')">${funcion.func_name}</a> ||
                                </c:otherwise>
                            </c:choose>
                        </c:forEach>
                    </td>
            ...
            <script type="text/javascript">
                ...
                function page(n) {
                    var href = "${ctx}/admin/dict/type/list" + "?currentPage=" + n + "&code_type=${retInfo.obj.code_type}&father_id=${funcions.obj.father_id}";
                    $.ajax({
                        type:'GET',
                        url:href,
                        dataType:'html',
