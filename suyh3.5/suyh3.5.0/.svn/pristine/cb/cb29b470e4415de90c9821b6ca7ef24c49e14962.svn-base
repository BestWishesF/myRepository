...
public class SignFilter implements Filter {
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
        ...
        if (!("3".equals(client_type) || "2".equals(client_type) || "1".equals(client_type) || "4".equals(client_type))) {
        ...
        if (url.indexOf(".") == -1) {
            String Key = "hotol";
            String Timestamp = AESUtils.decrypt(request.getHeader("Timestamp"), Constant.AES_KEY);
            String SignInfo = AESUtils.decrypt(request.getHeader("SignInfo"), Constant.AES_KEY);
                        ...
                        if (url.indexOf("/agentToken") > 0) {
                            ...
                            if (token != null) {
                                ...
                                if (tdHtAgentDto == null) {
                                    ...
                                    MemcachedUtils.replace(tdHtAgentDto.getToken(), tdHtAgentDto, new Date(20 * 24 * 60 * 60 * 1000));
                                }
                            } else {
                                String tip = "{\"tip\":" + "\"未登陆\"" + ",\"mark\":\"101\"}";
                                errorResponse(response, tip);
...
//		}
    ...
    private void errorResponse(HttpServletResponse response, String retStr) throws IOException {
        ...
        String encrypt = AESUtils.encrypt(retStr, Constant.AES_KEY);
