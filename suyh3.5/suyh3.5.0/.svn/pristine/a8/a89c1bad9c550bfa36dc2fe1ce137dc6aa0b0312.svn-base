...
<div class="Mask2" id="Mask2" style="display: none;position: fixed;">
    <div>
        <dl style="margin-left: 15px;">快件入库</dl>
        <hr />
        <dl class="fill_singleChoice">
            <input id="sclt_id" type="hidden">
            <p>快递类型:
                <select id="exp_ord_type">
                </select>
            </p>
            <p id="express_number_p" style="display: none;">
                快递单号:
    ...
    <div class="styleButton" style="text-align: center;">
        <button class="btn_b" id="sbtn_save">保存</button>
        <button class="btn_b" id="sbtn_cancel">取消</button>
    </div>
</div>
    ...
    <div class="centercontent tables">
        <div id="contentwrapper" class="contentwrapper">
                        ...
                        <c:forEach items="${funcions.obj.sonFuncion}" var="funcion">
                            <c:choose>
                                ...
                                <c:when test="${funcion.func_name == '入库'}">
                                    <c:if test="${bean.express_number == null||bean.express_number == ''}">
                                        <a class="btn_a" onclick="storageExp(${bean.exp_ord_clt_id})" href="###">入库</a>||
                                    </c:if>
                                </c:when>
            ...
            <script type="text/javascript">
                ...
                function page(n) {
                    var href =  "${ctx}/admin/express/order/collect" + "?currentPage=" + n +"&exp_ord_state=${retInfo.obj.exp_ord_state}&exp_ord_id=${retInfo.obj.exp_ord_id}&memb_type=4&father_id=${funcions.obj.father_id}"  ;
            ...
            <script type="text/javascript">
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.mark == "0") {
                                ...
                                var href =  "${ctx}/admin/express/order/collect" + "?currentPage=1&exp_ord_state=${retInfo.obj.exp_ord_state}&exp_ord_id=${retInfo.obj.exp_ord_id}&memb_type=4&father_id=${funcions.obj.father_id}"  ;
                ...
                function storageExp(id) {
                    ...
                    $("#express_number_p").hide();
                }

                $("#exp_ord_type").change(function () {
                    if($("#exp_ord_type").val()==2){
                        $("#express_number_p").show();
                    }else {
                        $("#express_number_p").hide();
                    }
                })

                $("#sbtn_cancel").click(function () {
                    $("#exp_ord_zhezhao").hide();
                    $("#Mask2").hide();
                })
                ...
                $("#sbtn_save").click(function () {
                    ...
                    var data="exp_ord_clt_id="+$("#sclt_id").val()+"&exp_ord_type="+$("#exp_ord_type").val()+"&express_number="+express_number;
                    $.ajax({
                        type: 'post',
                        url: '${ctx}/admin/express/bigcustomer/storage',
                        dataType: 'json',
                        data: data,
                        contentType: "application/x-www-form-urlencoded; charset=utf-8",
                        success: function (data) {
